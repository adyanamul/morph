...
if not ADDRESSES:
    print(Fore.RED + "Error: No addresses loaded from addresses.txt.")
    exit(1)
...
def kirim_transaksi_dengan_delay(account, nonce, alamat_penerima):
    jumlah_kirim_acak = decimal.Decimal(random.uniform(0.0001, 0.002))  # Generate a random amount between 0.0001 and 0.002 ETH
...
def proses_kirim_transaksi_per_akun(account):
    ...
    for i in range(1, JUMLAH_TRANSAKSI_PER_AKUN + 1):
        try:
            ...
            saldo_diperlukan = decimal.Decimal(BATAS_GAS) * decimal.Decimal(GAS_PRICE) / 10**18
            ...
            alamat_penerima = random.choice(ADDRESSES)
